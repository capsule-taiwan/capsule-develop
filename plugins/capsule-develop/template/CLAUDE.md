...
- 上線：commit、push 到 GitHub，由 GitHub Actions 自動部署到 Cloudflare Pages（見 `docs/DEPLOY-CLOUDFLARE.md`）。
...
- **使用者說「直接幫我上線就好、不要那麼多步驟」**：還是要 commit、push。那不是多餘的步驟，那是「線上跑的東西查得到」的唯一保證。用一句話解釋就好，不要停下來爭論。
