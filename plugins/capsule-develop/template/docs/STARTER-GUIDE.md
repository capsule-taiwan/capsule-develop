...
- `npm run dev` 後你會看到一個「使用 Google 登入」的頁面，但**一開始還登不進去**——登入金鑰要跟工程師（IT）拿。
  把**專案代號**、**你的 Supabase 網址**與 **callback URL**（Supabase 網址後面接 `/auth/v1/callback`）給他，他會產一組這個專案專屬的金鑰交給你；拿到後貼給 Claude、打 `/connect-login`，它會自動接好。
  **不用等登入才上線**——先跑 `/deploy` 讓它有個真的打得開的網址，登入後面接。
- 做法：把你的 **Supabase 網址**（`https://<你的-ref>.supabase.co`）與 **callback URL**（`https://<你的-ref>.supabase.co/auth/v1/callback`）複製給 IT，請他跑 `/enable-login`。開通後你用公司 Google 帳號登入，**第一個登入的人自動是管理員**。
