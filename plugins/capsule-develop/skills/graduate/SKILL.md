...
2. 掃描這個專案，逐項檢查回收契約遵守狀況，產出報告 `docs/graduation-packet.md`：
   ...
   - **程式碼與上線履歷**：這個 MVP 的 GitHub repo 網址（`git remote get-url origin`），以及線上跑的是哪一個 commit。
     上線一律走 GitHub 自動部署，所以每一版線上程式碼都對得到一個 commit——把 repo 交出去，平台團隊就看得到完整歷程。
     真的出現「線上有、repo 沒有」的落差，在申請包裡照實寫出來，那是回收前要先補的項目。
   - **搬遷影響**：列出這個 MVP 用到、但主系統可能不同的東西（例如：獨立的個人 Supabase 要併入公司帳號、這個 MVP 專屬的 Google OAuth client 換成母艦自己的登入設定）。MVP 已是公司 Google 登入，`useAuth`/`usePermissions` 介面與母艦同名，改動小。
   - **待撤銷項目**：畢業後這個 MVP 的專屬 OAuth client（`mvp-<專案代號>`）與開發者的個人 Supabase 專案都該收掉，請在申請包裡列出來提醒 IT（撤銷步驟見 `skills/enable-login/SKILL.md` 的「撤銷」段）。
